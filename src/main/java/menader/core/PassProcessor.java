...
public class PassProcessor extends AbstractProcessor {
	...
	public void writePassFile(List<Element> methods, TypeElement clazz, String packageName, String className) throws IOException {
		...
		var apply = MethodSpec.methodBuilder("apply")
			...
			.addStatement("$L $L = new $L()", clazz.getSimpleName(), varName, clazz.getSimpleName());
		...
		var passImpl = TypeSpec.classBuilder(className)
	...
	public void generateSafePasses(Class<? extends Annotation> annotation, RoundEnvironment env) {
		for (var elem : env.getElementsAnnotatedWith(annotation)) {
			...
			if (elem.getKind() != ElementKind.CLASS) {
				processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "You can use this annotation only on classes not interfaces or enums!");
				return;
			}
			...
			for (var enclosedElem : typeElem.getEnclosedElements()) {
				if (enclosedElem.getAnnotation(XMLSelect.class) != null) {
					if (enclosedElem.getKind() != ElementKind.METHOD) {
						processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "The XMLPath annotation can only be used on methods");
					}
					var methodElem = (ExecutableElement) enclosedElem;
					if (methodElem.getReturnType().getKind() != TypeKind.VOID) {
						processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR, "Methods with the XMLSelector annoation have to return void.");
					}
					methods.add(enclosedElem);
				}
			}

			String packageName = processingEnv.getElementUtils().getPackageOf(typeElem.getEnclosingElement()).toString();
	...
	public void writePassMgrFile(String packageName) throws IOException {
		var applySafe = MethodSpec.methodBuilder("applySafe")
			.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
			.returns(void.class)
			.addParameter(Document.class, "doc")
			.addParameter(Marshaller.class, "m");
		...
		var applyUnsafe = MethodSpec.methodBuilder("applyUnsafe")
			.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
			.returns(void.class)
			.addParameter(Document.class, "doc")
			.addParameter(Marshaller.class, "m");
		...
		var passMgrImpl = TypeSpec.classBuilder("PassManager")
			.addModifiers(Modifier.PUBLIC, Modifier.FINAL)
