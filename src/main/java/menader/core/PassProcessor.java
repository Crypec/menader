...
public class PassProcessor extends AbstractProcessor {
  ...
  public void writePassMgrFile(String packageName) throws IOException {
	...
	public void writeValidatorFile(String packageName) throws IOException {
		...
		var buildValCache = MethodSpec.methodBuilder("buildValidationCache")
			.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
			.addParameter(Document.class, "doc")
			.addStatement("$T<$T, $T> cache = new $T<$T, $T>()", HashMap.class, String.class, String.class, HashMap.class, String.class, String.class)
			.returns(cacheType);
		...
		var treeWalk = MethodSpec.methodBuilder("treeWalk")
			.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
			...
			.addStatement("$T topLevelMsg = $T.on($S).yellow().bold().toString() + $T.on($S).yellow().bold().toString() + $T.on($S + score + $S).red().bold().toString() + $T.on(child.getText()).red().bold().toString() + $S + $T.on(entry.getKey()).red().bold().toString()", String.class, Chalk.class, "[Warning]", Chalk.class, " Found potentially unremoved PII ", Chalk.class, "[", "]: ", Chalk.class, " == ", Chalk.class)
			.addStatement("$T.out.println(topLevelMsg)", System.class)
			.addStatement("$T msg = $T.on($S).yellow().bold().toString() + $T.on($S).yellow().toString() + $T.on(entry.getValue()).magenta().toString()", String.class, Chalk.class, "[Warning]: ", Chalk.class, "This should have been removed by this xPath expr: ", Chalk.class)
			.addStatement("$T.out.println(msg)", System.class)
			.addStatement("$T hint = $T.on($S).green().bold().toString() + $T.on($S).yellow().toString() + $T.on(child.getName()).magenta().toString()", String.class, Chalk.class, "[Hint]: ", Chalk.class, "The PII was found in a node with the name: ", Chalk.class)
		...
		var validate = MethodSpec.methodBuilder("validate")
			...
			.addParameter(Document.class, "doc")
			.addParameter(float.class, "wordSimilarityTreshold")
			.addStatement("$T<$T, $T> cache = $L(doc)", HashMap.class, String.class, String.class, buildValCache.build().name)
		...
		var validateOverLoad = MethodSpec.methodBuilder("validate")
			.addModifiers(Modifier.PUBLIC, Modifier.STATIC)
			.addParameter(Document.class, "doc")
			.addStatement("validate(doc, $Lf)", WORD_SIMILARITY_TRESHOLD);

		var metricField = FieldSpec.builder(StringMetric.class, "metric")
			.addModifiers(Modifier.PRIVATE, Modifier.STATIC, Modifier.FINAL)
				.initializer(
						...
						StringMetricBuilder.class, DamerauLevenshtein.class, Simplifiers.class, Locale.class,
						Simplifiers.class, Locale.class, Simplifiers.class);

		var passValidatorImpl = TypeSpec.classBuilder("PassValidator")
